        ...
        for element in self.soprano_part.parts[0].getElementsByClass(stream.Measure):
            ...
            for note in element.notes:
                ...
                if note.name not in self.harmony_dict:
                    note_name = note.getEnharmonic().name
                else:
                    note_name = note.name
...
class FitnessEvaluator:
    ...
    def get_best_harmony(self, chorales):
        return max(chorales, key=self.evaluate)
...
def convert_to_chorale_style(score):
    ...
    metronome_mark = tempo.MetronomeMark(number=60)
    treble_staff.append(metronome_mark)
    bass_staff.append(metronome_mark)
...
def preprocess_chorale(chorale):
    ...
    soprano.id = 'Soprano'
    soprano.clef = music21.clef.TrebleClef()

    # create empty Alto, Tenor, and Bass parts
    alto = music21.stream.Part(id='Alto')
...
def main():
    XMLfile = "./testing-chorale.xml"
    data = music21.converter.parse(XMLfile)
  
    # # load all Bach chorales and randomly select one
    # # uncomment this to try an actual bach soprano line:
    # bach_chorales = music21.corpus.getComposer('bach')
    # selected_chorale_path = random.choice(bach_chorales)
    # chorale = music21.converter.parse(selected_chorale_path)
    # data = preprocess_chorale(chorale)
    ...
    fitness_evaluator = FitnessEvaluator(
        weights=weights,
        use_intervalic_qualities=True, # False
    )

    harmonizer = EvolutionaryChoraleHarmonizer(
        soprano_part=data,
        population_size=50,
        mutation_rate=0.05,
        fitness_evaluator=fitness_evaluator,
        verbose=True,
        encourage_diversity=True # False
    )
    
    generated_chorale = harmonizer.harmonize(generations=10)
